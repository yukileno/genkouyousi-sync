    ...
    if (action === "save") {
      ...
      if (foundRow > 0) {
        ...
        if (studentId != 99 && studentId != "99") {
          ...
          // 作文の種類 (K列: 11), 題名 (L列: 12), 本の名前 (M列: 13) を保存
          ...
          if (params.bookName !== undefined) {
            sheet.getRange(foundRow, 13).setValue(params.bookName);
          }
        }
...
function getTargetSheet(classNumber) {
  ...
  if (sheetId) {
    try {
      ...
      if (sheet.getLastRow() === 0) {
        sheet.appendRow(["クラス", "出席番号", "氏名", "パスワード", "最終保存日時", "文字数", "本文", "設定データ", "先生のアドバイス", "できたフラグ", "作文の種類", "題名", "本の名前"]);
      }
      return sheet;
    } catch(err) {
  ...
  if (defaultSheet.getLastRow() === 0) {
    defaultSheet.appendRow(["クラス", "出席番号", "氏名", "パスワード", "最終保存日時", "文字数", "本文", "設定データ", "先生のアドバイス", "できたフラグ", "作文の種類", "題名", "本の名前"]);
  }
  return defaultSheet;
}
