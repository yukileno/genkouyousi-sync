...
//        .setRuby(["げん","こう","よう","し","","","","","","あいうえお"])
        ...
        // Login UI
        this.$dialogLogin = $("#dialog-login");
        this.$btnLogin = $("#btn-login").click(this.onLoginClicked.bind(this));
        this.$btnLogout = $("#logoutBtn").click(this.onLogoutClicked.bind(this));
        this.$btnLogoutStudent = $("#logoutBtn-student").click(this.onLogoutClicked.bind(this));
        this.$studentSelector = $("#student-selector").change(this.onStudentSelected.bind(this));
    ...
    async onLoginClicked() {
        const attendanceNumber = $("#login-id").val().trim();
        const pwd = $("#login-password").val().trim();
        if (attendanceNumber === "") {
            $("#login-error").text("出席番号を入力してください。").show();
            return;
        }

        this.$btnLogin.prop("disabled", true);
        const result = await this.saveHandler.login(attendanceNumber, pwd);
        this.$btnLogin.prop("disabled", false);

        if (result.success) {
            this.$dialogLogin.modal('hide');
            $("#account-panel").css("display", "flex");
            
            if (result.isTeacher) {
                $("#teacher-panel").css("display", "flex");
                $("#student-panel").hide();
                await this.initTeacherPanel();
            } else {
                $("#student-panel").css("display", "flex");
                $("#teacher-panel").hide();
                $("#current-student-name").text(`${result.studentName || attendanceNumber + '番'} さん`);
    ...
    async initTeacherPanel() {
        const students = await this.saveHandler.getStudents();
        this.$studentSelector.empty().append('<option value="">児童を選択...</option>');
        for (const st of students) {
            // st could be an object {attendanceNumber: 1, name: "あああ"} or just a string/number
            const num = st.attendanceNumber !== undefined ? st.attendanceNumber : st;
            const name = st.name ? st.name : `${num}番`;
            this.$studentSelector.append(`<option value="${num}">${num} ${name}</option>`);
        }
        // 初期状態は空の原稿にする
        this.genko.setText("");
        this.genko.render();
    ...
    async onStudentSelected() {
        const studentId = this.$studentSelector.val();
        if (!studentId) {
            this.saveHandler.currentViewingStudent = null;
            this.genko.setText("");
            this.genko.render();
            return;
        }
        ...
        this.startProcessing("backdrop-opening");
        // 前の児童の状態を保存してから切り替えるか？
        // ここでは都度自動保存されているという前提で切り替え
        this.saveHandler.currentViewingStudent = studentId;
        await this.saveHandler.load();
        this.endProcessing();
    }

    onLogoutClicked() {
        this.saveHandler.logout();
        $("#account-panel").hide();
        this.showLogin();
    }
